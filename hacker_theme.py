...
banner = banner.replace('meghamittal563@gmail.com', 'mullamasoom50@gmail.com')
...
<g class="ltr" style="animation-delay:1.5s">
  <text x="48" y="185" font-family="'Comic Sans MS', 'Brush Script MT', cursive, sans-serif" font-style="italic" font-size="65" font-weight="bold" fill="url(#nameg)" filter="url(#glow)">MASOOM🤍</text>
  <text x="48" y="185" font-family="'Comic Sans MS', 'Brush Script MT', cursive, sans-serif" font-style="italic" font-size="65" font-weight="bold" fill="url(#nameg)">MASOOM🤍</text>
</g>'''
    banner = banner[:idx1] + masoom_name_svg + banner[idx2:]

# Update portfolio text
banner = banner.replace("I don't watch anime,", "I don't just write code,")
banner = banner.replace('I <tspan fill="#ff7eb6" font-weight="bold">code</tspan><tspan fill="#e6edf3"> anime.</tspan>', 'I <tspan fill="#4ade80" font-weight="bold">craft</tspan><tspan fill="#e6edf3"> experiences.</tspan>')
banner = banner.replace("Code is my art, Logic is my superpower.", "Striving to grow my skills every day. 🤍")

banner = banner.replace("&lt; Frontend Developer /&gt;", "&lt; Frontend Dev &amp; UI/UX /&gt;")
banner = banner.replace("&lt; JavaScript Enthusiast /&gt;", "&lt; Designer &amp; Engineer /&gt;")
banner = banner.replace("&lt; SQL  SAP Workflows /&gt;", "&lt; Student &amp; Learner /&gt;")
banner = banner.replace("&lt; Anime Website Creator /&gt;", "&lt; Web App Developer /&gt;")

banner = banner.replace("I build responsive, user-friendly and impactful web experiences.", "Aspiring web developer &amp; UI/UX designer from Karnataka.")
banner = banner.replace("Always learning, always building.", "Currently in B.E. Computer Science, always learning.")
banner = banner.replace("Turning ideas into real world solutions.", "Passionate about modern web development &amp; vibe coding.")

# Fix avatar crop logic! Set y=152 so it aligns with clip-path, and preserveAspectRatio to slice from the top (YMin)
banner_img = get_b64('banner.png')
img_pattern = re.compile(r'<image x="722" y="152" width="558" height="522" href="data:image/[^;]+;base64,[^"]+"')
new_img_tag = f'<image x="722" y="152" width="558" height="522" preserveAspectRatio="xMidYMin slice" href="{banner_img}"'
banner = img_pattern.sub(new_img_tag, banner)

# Hacker Green Color Palette Replacements
colors = {
    '#ff7eb6': '#4ade80',  # bright green
    '#c084fc': '#10b981',  # emerald green
    '#8b5cf6': '#059669',  # darker emerald
    '#e879f9': '#22c55e',  # solid green
    '#f9a8d4': '#86efac',  # light green
    '#f0abfc': '#34d399',  
    '#c4b5fd': '#6ee7b7',
    '#93c5fd': '#4ade80',
    '#67e8f9': '#a7f3d0',
    '#38bdf8': '#0ea5e9',  # keeping a slight cyber blue accent for orbB
    '#120b20': '#061208',  # background
    '#170e28': '#09180c',  # background
    '#0e0918': '#040a05',  # background
    'rgba(255,126,182,.10)': 'rgba(74,222,128,.10)',
    'rgba(139,92,246,.14)': 'rgba(16,185,129,.14)',
    'rgba(0,118,206,.14)': 'rgba(34,197,94,.14)',
    'rgba(97,218,251,.10)': 'rgba(74,222,128,.10)',
    '#231541': '#09180c',  # lanyard header
    '#1d1330': '#061208',  # lanyard card
    '#140d22': '#040a05',  # lanyard card
    '#d6478f': '#10b981',  # lanyard strap
}
for old, new in colors.items():
    banner = banner.replace(old, new)
    lanyard = lanyard.replace(old, new)
...
print("SVGs generated successfully with Hacker Green Theme")
